...
PROFILE: dict = {
    # ===== meta =====
    "name": "kesulu_001",  # profile 名称（用于输出/日志标识）
    "style": "kesulu",  # 风格标签（仅用于人类识别；流程逻辑不应依赖它）

    # ===== spoken（Call#1：从 raw_text 生成口播稿） =====
    "spoken_system_1": tps.KESULU_SYS_PROMPT_001_02,  # system prompt
    "spoken_user_1_template": (
        # user prompt 模板：必须包含 {raw_text}
    ...
    "img_prompt_system": tps.KESULU_IMAGE_PROMPT_SYNC_PROMPT_001,  # system prompt（字符串）
    ...
    "img_prompt_base": tps.KESULU_IMAGE_PROMPT_BASE_001,  # base JSON（dict）：用于补齐缺失字段/统一风格字段
    ...
    "cloned_voice_id": "cosyvoice-v3-flash-manflash01-7cc91b1194ed4a4a982d035734709b8b",  # 克隆音色 ID（字符串）
